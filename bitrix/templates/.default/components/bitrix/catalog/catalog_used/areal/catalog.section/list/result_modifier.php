    ...
    foreach ($arResult["ITEMS"] as $key => $arItem)
    {
        ...
        if (!empty($arItem["PICTURE_ID"]))
        {
            $arResult["ITEMS"][$key]["PICTURE"] = CFile::ResizeImageGet(
                            $arItem["PICTURE_ID"], array("width" => 100, "height" => 50), BX_RESIZE_IMAGE_PROPORTIONAL, true
            );
        }
        else
        {
            if (!empty($arItem["PROPERTIES"]["SERIES"]["VALUE"]))
            {
                ...
                if ($series = $serieses->GetNext())
                {
                    ...
                    {
                        $arResult["ITEMS"][$key]["PICTURE"] = CFile::ResizeImageGet(
                                        $series["PREVIEW_PICTURE"], array("width" => 100, "height" => 50), BX_RESIZE_IMAGE_PROPORTIONAL, true
                        );
                    }
                }
            }
            elseif (!empty($arItem["IBLOCK_SECTION_ID"]))
            {
                ...
                if ($section = $sections->GetNext())
                    $arResult["ITEMS"][$key]["PICTURE"] = CFile::ResizeImageGet(
                                    $section["PICTURE"], array("width" => 100, "height" => 50), BX_RESIZE_IMAGE_PROPORTIONAL, true
                    );
            }
        }
        ...
        {
            $arResult["ITEMS"][$key]["PICTURE"] = CFile::ResizeImageGet(
                            $arItem["DETAIL_PICTURE"], array("width" => 100, "height" => 50), BX_RESIZE_IMAGE_PROPORTIONAL, true
            );
        }
        ...
        else
        {
            ...
            if ($section = $sections->GetNext())
            {
               $arResult["ITEMS"][$key]["DETAIL_PAGE_URL"] =$section["SECTION_PAGE_URL"].$arResult["ITEMS"][$key]["CODE"]."/";
            }
        }
