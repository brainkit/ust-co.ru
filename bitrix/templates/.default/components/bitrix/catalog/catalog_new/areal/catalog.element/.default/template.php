...
?>
				<a class="fancybox" href="<?=$image_path?>" >
			<?endif;?>
					<table><tr><td>
						<?if(!empty($arResult["PHOTOS"][0]["STANDART"]["src"])):?>
							<img src="<?=$image_path?>" width="<?=$arResult["PHOTOS"][0]["STANDART"]["width"]?>" height="<?=$arResult["PHOTOS"][0]["STANDART"]["height"]?>" alt="<?=$arResult["NAME"]?>" title="<?=$arResult["NAME"]?>" />
						<?else:?>
									...
									<thead>
										<tr>
											...
											<?foreach($arResult["ITEMS"] as $arItem):?>				
												<th class="name">
													<label class="checkbox label_check">
														<input type="checkbox" name="compare_<?=$arItem["ID"]?>" value="<?=$arItem["ID"]?>"><?=$arItem["SHORT_NAME"]?>
													</label>
												</th>
											<?endforeach;?>
										</tr>
