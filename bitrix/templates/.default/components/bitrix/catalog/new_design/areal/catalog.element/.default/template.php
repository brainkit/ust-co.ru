...
?> hproduct vert_design"  id="<? echo $this->GetEditAreaId($arResult['ID']); ?>">
    ...
    <div class="ppic detail">
        ...
        <?
        //SEO CODE
        //выводим значок Союза Недропользователей России 
        if ($arResult['SECTION']['PATH'][0]['CODE']=="burovoe-i-svaeboynoe-oborudovanie" or $arResult['SECTION']['PATH'][0]['CODE']=="drobilno-sortirovochnoe-oborudovanie" or $arResult['SECTION']['PATH'][1]['CODE']=="karernye-samosvaly") echo '<div class="soyuz-nedra"><img src="/images/soyuz-nedra.png" width="140" height="79" alt="Универсал-Спецтехника - участник Союза Недропользователей России" title="Универсал-Спецтехника - участник Союза Недропользователей России"></div>';
...
<? if (count($arResult["PHOTOS"]) > 1): ?>
        ...
        <? if ($key > 0): ?>
                            <li>
                                ...
                                ?>
                                <ahref="<?= $image_path ?>" class="image <? if ($key == 0): ?>active<? endif; ?> fancybox-thumb">
            ...
            <ul>
                <? foreach ($arResult["PROPERTIES"]['SERTIFICATE']['VALUE'] as $key => $sertificate): ?>
                    <li>
                     ...
                     <?//echo 'Скачать '. ($arResult["PROPERTIES"]['SERTIFICATE']['DESCRIPTION'][$key]) . ': <a class="pdf_ico" href="'. (CFile::GetPath($sertificate)) .'">&nbsp;</a>';?>
                    
                    <a href="<? echo CFile::GetPath($sertificate); ?>" target="_blank" title="<?= $arResult["PROPERTIES"]['SERTIFICATE']['DESCRIPTION'][$key] ?>" class="pdf_ico_small">Скачать <?= $arResult["PROPERTIES"]['SERTIFICATE']['DESCRIPTION'][$key] ?></a>
...
//SEO CODE
            if ($trans_brand_name != "sdmo" and $trans_brand_name != "transprogress") {
                ...
                $newstr = preg_replace("/ ЛЦ/"," ЛК",$newstr); 
...
//SEO CODE END
    ...
    <? if (!empty($arResult["VIDEO"])): ?>
        ...
        <? foreach ($arResult["VIDEO"] as $key => $arVideo): ?>
                        <div class="item-video <? if (($key + 1) % 3 == 0): ?> last <? endif; ?><? if (($key) % 3 == 0): ?> first <? endif; ?>">
                            <div class="video">
                                <a class="video_show image" href="#" title="video_<?= $arVideo["ID"] ?>">
    ...
    <? if (!empty($arResult['SCHEMES'])): ?>
        ...
        <? foreach ($arResult["SCHEMES"] as $key => $arScheme): ?>
                                <li>
                                    <a href="<?= $arScheme["NATURE"] ?>" class="image <? if ($key == 0): ?>active<? endif; ?> fancybox-thumb">
...
<? endif; ?>
    <div class="clear"></div>
        ...
        <?
        //берем код модели, привязанной к карточке. свойство "Привязка к модели запчастей"
        //if ($USER->IsAdmin()) {
           $db_props = CIBlockElement::GetProperty($arResult["IBLOCK_ID"], $arResult["ID"], array("sort" => "asc"), Array("CODE" => "zapchasti"));
           if ($ar_props = $db_props->Fetch())
               $model_id = $ar_props["VALUE"];
           $APPLICATION->IncludeComponent("unispec:catalog_element.zapchasti", ".default", array("ID_MODEL" => $model_id, "ITEM_NAME" => $arResult["NAME"], "BRAND_NAME" =>$brand_name, "BRAND_ID" =>$brand_id));
        //}
        ?>
...
<? endif; ?> 
    ...
    <div class="useful_hrefs">Смотрите также: 
        ...
        <? endif; ?>
            <?//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/ajax/elem.txt', print_r($show_in_href,true));?>
