...
<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
	<input type="checkbox" name="RUB_ID[]" value="<?=$itemValue["ID"]?>" id="radio<?=$itemValue["ID"]?>" class="css-checkbox" <?if($itemValue["CHECKED"]) echo " checked"?> />
	<label for="radio<?=$itemValue["ID"]?>" class="css-label radGroup2"><?=$itemValue["NAME"]?></label><br />

	<!-- <input type="checkbox" class="subscr_checkbox" name="RUB_ID[]" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo " checked"?> /><?=$itemValue["NAME"]?><br /> -->
<?endforeach;?>
<input type="text" class="subscr_input" name="EMAIL" value="<?=$arResult["SUBSCRIPTION"]["EMAIL"]!=""?$arResult["SUBSCRIPTION"]["EMAIL"]:$arResult["REQUEST"]["EMAIL"];?>" size="30" maxlength="255" placeholder="email*"/><br />
<input type="text" class="subscr_input" name="USER_NAME" value="<?if ($userName) echo $userName; else echo "";?>" size="30" maxlength="255" placeholder="ФИО"/>
<? if(false):?>
...
<input type="submit" class="subscr_submit" name="Save" value="Сохранить настройки" />
<input type="hidden" name="PostAction" value="<?echo ($arResult["ID"]>0? "Update":"Add")?>" />
<input type="hidden" name="ID" value="<?echo $arResult["SUBSCRIPTION"]["ID"];?>" />
<?if($_REQUEST["register"] == "YES"):?>
	<input type="hidden" name="register" value="YES" />
