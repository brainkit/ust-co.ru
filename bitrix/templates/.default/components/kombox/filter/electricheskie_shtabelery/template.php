...
if (!empty($arr_filter_brand) and count($arr_filter_brand)>1) {
   foreach ($arr_filter_brand as $item_filter) {
      echo '<a href="http://ust-co.ru/catalog/elektricheskie-shtabelery/filter/brand-'.$item_filter["key"].'/">электрические штабелеры  '.$item_filter["val"].'</a>';
      echo ', ';
...
if (!empty($arr_filter_ispoln) and count($arr_filter_ispoln)>1) {
   ...
   foreach ($arr_filter_ispoln as $item_filter) {
      echo '<a href="http://ust-co.ru/catalog/elektricheskie-shtabelery/filter/pto_tip_privoda-'.$item_filter["key"].'/">электрический штабелер '.(mb_strtolower($item_filter["val"])).'</a>';
