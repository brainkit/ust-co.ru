...
// seo code end
    ...
    <? elseif ((count($arResult["ITEMS"]["212"]["VALUES"]) <= COption::GetOptionInt("ust", "catalog_filter_count_brand")) && ($arResult["ITEMS"]["212"]["CODE_ALT"] == "brand") && (count($arResult["ITEMS"]["212"]["VALUES"]) != 1)): ?>
                    <li class="lvl1 select_left " data-id="<? echo $arResult["ITEMS"]["212"]["CODE_ALT"] . '-' . $arResult["ITEMS"]["212"]["ID"] ?>">
                        ...
                        <div class="kombox-combo brand-check" data-name="<?= $arResult["ITEMS"]["212"]["CODE_ALT"] ?>">                
                            <ul>
        <? foreach ($arResult["ITEMS"]["212"]["VALUES"] as $val => $ar): ?>
                                    <li class="li_checkbox <? echo $ar["DISABLED"] ? 'kombox-disabled ' : '' ?><? echo $ar["CHECKED"] ? 'kombox-checked' : '' ?>">                               
                                        <input
            ...
            <? echo $ar["CHECKED"] ? 'checked="checked"' : '' ?>
                                        ...
                                        <? if (!$arResult["ITEMS"]["212"]["ID"]): ?>
                                            <label for="<?= $ar["CONTROL_ID"] ?>" class="label_checked checkbox_on"><span style="display: inline-block !important;
    padding-left: 20px;
    text-align: left;
    top: 3px;
             position: relative;
             top: 8px !important;
             font-size: 12px"><? echo $ar["VALUE"]; ?></span> <span>(<? echo $arResult["section_filter"]["CNT"]; ?>)</span></label>
                                        <? else: ?>
                                            <label for="<?= $ar["CONTROL_ID"] ?>" class="label_checked checkbox_off"><span style="display: inline-block !important;
    padding-left: 20px;
    text-align: left;
    top: 3px;
             position: relative;
             top: 8px !important;
             font-size: 12px"><? echo $ar["VALUE"]; ?></span> <span>(<? echo $ar["CNT"]; ?>)</span></label>
                                    <? endif ?>
                                    </li>
                                    <?
                                    // seo code
...
//brand links
    ...
    if (!empty($arr_filter_brand) and count($arr_filter_brand) > 1) {
        foreach ($arr_filter_brand as $item_filter) {
            echo '<a href="http://ust-co.ru/catalog/vilochnye-pogruzchiki/filter/brand-' . $item_filter["key"] . '/">вилочный погрузчик ' . $item_filter["val"] . '</a>';
            echo ', ';
        }
    }
    if (!empty($arr_filter_ispoln) and count($arr_filter_ispoln) > 1) {
        ...
        foreach ($arr_filter_ispoln as $item_filter) {
            echo '<a href="http://ust-co.ru/catalog/vilochnye-pogruzchiki/filter/pto_tip_privoda-' . $item_filter["key"] . '/">вилочный погрузчик ' . (mb_strtolower($item_filter["val"])) . '</a>';
            if ($j < count($arr_filter_ispoln))
                echo ', ';
            $j++;
        }
    }
