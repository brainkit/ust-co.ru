...
// print_r($arResult);
// echo "</pre>";
// file_put_contents($_SERVER['DOCUMENT_ROOT'].'/ajax/testKomboxStroi.txt', print_r($arResult,true));
            ...
            <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> <?if($element_conter%2):?>select_right<?else:?>select_left<?endif?>" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
                ...
                <div id="brand_holder" style="padding: 5px 0 0 15px;">
                    <select name="<?=$arItem["CODE_ALT"]?>" class="kombox-select-element" style="width: 324px; padding: 5px 0 0 15px;">
                        ...
                        <?foreach($arItem["VALUES"] as $val => $ar):?>
                            <option class="kombox_option-element"  style="width: 324px;" id="<?=$ar["CONTROL_NAME_ALT"];?>" value="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?></option>
                        <?endforeach;?>
                    </select>
...
if (!empty($arr_filter_brand) and count($arr_filter_brand)>1) {
   ...
   foreach ($arr_filter_brand as $item_filter) {
      echo '<a href="http://u-st.ru/catalog/stroitelnaya-tekhnika/filter/brand-'.$item_filter["key"].'/">'.$item_filter["val"].'</a>';
