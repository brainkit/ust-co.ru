            ...
            <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> <?if($element_conter%2):?>select_right<?else:?>select_left<?endif?>" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
                ...
                <div class="kombox-combo" data-name="<?=$arItem["CODE_ALT"]?>" style="padding: 10px 0 0 0;">                
                    <ul>
                        <?$element_conter_li = 0;
                        foreach($arItem["VALUES"] as $val => $ar):?>
                        <li class="li_checkbox <?if($element_conter_li%2):?>select_right<?else:?>select_left<?endif?> <?echo $ar["DISABLED"]? 'kombox-disabled ': ''?> <?echo $ar["CHECKED"]? 'kombox-checked': ''?>" style="width: <?if($element_conter_li%2):?>110px<?else:?>120px<?endif?>;">                               
                            <input
                                class="kombox-hidden-input"     
                                type="checkbox"
                                value="<?echo $ar["HTML_VALUE"]?>"
