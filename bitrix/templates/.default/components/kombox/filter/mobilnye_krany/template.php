...
//echo "</pre>";?>
<?//unset($_POST);?>
<div class="kombox-filter mobilnye-krany" id="kombox-filter">
            ...
            <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> <?if($element_conter%2):?>select_right<?else:?>select_left<?endif?>" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
                ...
                <div class="kombox-combo" data-name="<?=$arItem["CODE_ALT"]?>">                
                    <ul>
                        <?$element_conter_li = 0;
                        foreach($arItem["VALUES"] as $val => $ar):?>
                        <li class="li_checkbox <?if($element_conter_li%2):?>select_right<?else:?>select_left<?endif?><?echo $ar["DISABLED"]? 'kombox-disabled ': ''?><?echo $ar["CHECKED"]? 'kombox-checked': ''?>">                               
                            <input
                                class="kombox-hidden-input"     
                                type="checkbox"
                                value="<?echo $ar["HTML_VALUE"]?>"
