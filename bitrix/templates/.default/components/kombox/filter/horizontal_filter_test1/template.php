            ...
            <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> <?if($element_conter%2):?>select_left<?else:?>select_right<?endif?>" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
                ...
                <div style="padding: 5px 0 0 15px;">
                    <select name="<?=$arItem["CODE_ALT"]?>" class="kombox-select-element" style="width: 324px; padding: 5px 0 0 15px;">
                        ...
                        <?foreach($arItem["VALUES"] as $val => $ar):?>
                            <option class="kombox_option-element"  style="width: 324px;" id="<?=$ar["CONTROL_NAME_ALT"];?>" value="<?=$ar["VALUE"];?>" <?echo $ar["CHECKED"]? 'selected="selected"': ''?>><?=$ar["VALUE"];?></option>
                        <?endforeach;?>
                    </select>
...
<script>
    $(document).ready(function(){
        $("a.kombox-del-filter").click(function(){
            window.location = "<?=$arResult["DELETE_URL"]?>";
        });
    });
</script>
<!--<div id="chosen"></div>-->