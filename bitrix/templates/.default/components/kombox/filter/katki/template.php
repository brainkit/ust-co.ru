    ...
    <form name="<?echo $arParams["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get"<?if($arResult['IS_SEF']):?> data-sef="yes"<?endif;?>>
        ...
        <?if($arItem["PROPERTY_TYPE"] == "N" || isset($arItem["PRICE"])):?>
            ...
            <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> select_right" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
                ...
                <div class="solo_brand"><?=(int)$arItem["VALUES"]["MIN"]["VALUE"];?></div>
            </li> 
            <?else:?>
            <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> select_right" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
                <div class="kombox-filter-property-head kombox-middle" style="width: 147px; padding: 1.8em 10px 5px 5px;">
            ...
            <?endif;?>
        <?elseif(($arItem["CODE_ALT"] == "tip_mashiny") && (!empty($arItem["VALUES"]))):?>
        <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> tip-mashiny-katki" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
                ...
                <div class="kombox-combo" data-name="<?=$arItem["CODE_ALT"]?>-katki"  >                
                    ...
                    <ul>
                        ...
                        <li class="li_checkbox <?echo $ar["DISABLED"]? 'kombox-disabled ': ''?><?echo $ar["CHECKED"]? 'kombox-checked': ''?>">                               
                            ...
                            />
                            <?if(!$arItem["ID"]):?>
                            <label for="<?=$ar["CONTROL_ID"]?>" class="label_checked checkbox_off"><b style="display: inline-block;
    font-weight: normal;
    padding-left: 20px;
    padding-top: 8px;"><?echo $ar["VALUE"];?></b> <span style="padding-top: 8px; display: none;">(<?echo $arResult["section_filter"]["CNT"];?>)</span></label>
                            <?else:?>
                            <label for="<?=$ar["CONTROL_ID"]?>" class="label_checked checkbox_off"><b style="display: inline-block;
    font-weight: normal;
    padding-left: 20px;
    padding-top: 8px;"><?echo $ar["VALUE"];?></b> <span style="padding-top: 8px; display: none;">(<?echo $ar["CNT"];?>)</span></label>
                            <?endif?>
                            <?/*<label for="<?echo $ar["CONTROL_ID"]?>"><?echo $ar["VALUE"];?> <span>(<?echo $ar["CNT"];?>)</span></label>*/?>
                        </li>
                        <?
                        // seo code
                        if ($arItem["CODE_ALT"]=="tip_mashiny") {
                               $arr_filter[]=array("key"=>$ar["CONTROL_NAME_ALT"],"val"=>$ar["VALUE"]);
                        }
                        // seo code end
                        ?>
                        <?endforeach;?>
        ...
        <?elseif(!empty($arItem["VALUES"]) && !isset($arItem["PRICE"])):?>
            <li class="lvl1<?if($arItem["CLOSED"]):?> closed<?endif;?> <?if($element_conter%2):?>select_right<?else:?>select_left<?endif?>" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>"<?if($arItem["CODE_ALT"] =="tip_mashiny"):?>style="width: 420px;"<?endif;?>>
                <div class="kombox-filter-property-head" style="padding: 1.8em 15px 5px 5px;">
                    <?if(strlen($arItem['HINT'])):?>
                    <span class="kombox-filter-property-hint"></span>
                    <div class="kombox-filter-property-hint-text"><?echo $arItem['HINT']?></div>
                    <?endif;?>
                ...
                <div class="kombox-combo" data-name="<?=$arItem["CODE_ALT"]?>"<?if($arItem["CODE_ALT"] =="tip_mashiny"):?>style="width: 290px;"<?endif;?>>                
                    <ul>
                        <?foreach($arItem["VALUES"] as $val => $ar):?>
                        <li class="li_checkbox <?echo $ar["DISABLED"]? 'kombox-disabled ': ''?><?echo $ar["CHECKED"]? 'kombox-checked': ''?>">                               
                            <input
                ...
                <div class="kombox-combo" data-name="<?=$arItem["CODE_ALT"]?>" style="padding: 10px 0 0 0;">                
                    <ul>
                        <?$element_conter_li = 0;
                        foreach($arItem["VALUES"] as $val => $ar):?>
                        <li class="li_checkbox <?if($element_conter_li%2):?>select_right<?else:?>select_left<?endif?> <?echo $ar["DISABLED"]? 'kombox-disabled ': ''?> <?echo $ar["CHECKED"]? 'kombox-checked': ''?>" style="width: <?if($element_conter_li%2):?>115px<?else:?>125px<?endif?>;">                               
                            <input
                                class="kombox-hidden-input"     
                                type="checkbox"
                                value="<?echo $ar["HTML_VALUE"]?>"
...
<?
//seo code
if (!empty($arr_filter) and count($arr_filter)>1) {
   
   echo '<div class="se_links">';
   $j=1;
   foreach ($arr_filter as $item_filter) {
      
      echo '<a href="http://u-st.ru/catalog/katki/filter/tip_mashiny-'.$item_filter["key"].'/">'.(mb_strtolower($item_filter["val"])).' каток</a>';
      if ($j < count($arr_filter)) echo ', ';
      $j++;
   }
   echo "</div>";
